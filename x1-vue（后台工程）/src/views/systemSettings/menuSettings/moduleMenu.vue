  ...
  <div>
    <el-card>
      <div>
        ...
        >
          ...
          <el-table-column prop="systemId" label="所属的系统菜单ID" min-width="140" />
          <el-table-column prop="title" label="模块名字" min-width="120" />
          <el-table-column prop="sort" label="排序位置" min-width="120" />
          <el-table-column prop="icon" label="图标" min-width="120" />
          <el-table-column prop="routeName" label="路由名字" min-width="140" />
    ...
    >
      <el-form :model="formData" label-width="150px">
        ...
        <el-form-item label="所属的系统菜单ID">
          <el-input v-model="formData.systemId" :disabled="true" />
        </el-form-item>
...
<script>
import { moduleMenu, updateModuleMenu } from '@/api/menuSetting'
...
export default {
  ...
  methods: {
    // 获取数据
    async getModuleMenu() {
      const res = await moduleMenu({
        page: this.currentPage,
        limit: this.pageSize
      })
      this.moduleData = res.data.data.data
    ...
    async handleConfirm() {
      ...
      const res = await updateModuleMenu(newData)
      this.getModuleMenu()
      if (res.data.code === 0) {
