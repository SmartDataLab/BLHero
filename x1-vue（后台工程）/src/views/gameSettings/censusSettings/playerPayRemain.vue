    ...
    <el-card>
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="day0" label="当日付费留存率" width="130">
            <template slot-scope="scope">
              {{ scope.row.day0 }}
              ({{ formatNumber(scope.row.day0/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day1" label="1日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day1 }}
              ({{ formatNumber(scope.row.day1/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day2" label="2日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day2 }}
              ({{ formatNumber(scope.row.day2/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day3" label="3日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day3 }}
              ({{ formatNumber(scope.row.day3/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day4" label="4日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day4 }}
              ({{ formatNumber(scope.row.day4/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day5" label="5日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day5 }}
              ({{ formatNumber(scope.row.day5/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day6" label="6日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day6 }}
              ({{ formatNumber(scope.row.day6/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day7" label="7日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day7 }}
              ({{ formatNumber(scope.row.day7/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day8" label="8日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day8 }}
              ({{ formatNumber(scope.row.day8/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day9" label="9日付费留存率" width="120">
            <template slot-scope="scope">
              {{ scope.row.day9 }}
              ({{ formatNumber(scope.row.day9/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day10" label="10日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day10 }}
              ({{ formatNumber(scope.row.day10/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day11" label="11日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day11 }}
              ({{ formatNumber(scope.row.day11/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day12" label="12日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day12 }}
              ({{ formatNumber(scope.row.day12/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day13" label="13日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day13 }}
              ({{ formatNumber(scope.row.day13/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day14" label="14日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day14 }}
              ({{ formatNumber(scope.row.day14/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day30" label="30日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day30 }}
              ({{ formatNumber(scope.row.day30/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day60" label="60日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day60 }}
              ({{ formatNumber(scope.row.day60/scope.row.dayBase*100) }}%)
            </template>
          </el-table-column>
          <el-table-column prop="day90" label="90日付费留存率" width="150">
            <template slot-scope="scope">
              {{ scope.row.day90 }}
...
import { playerPayRemain } from '@/api/gameStatistics'
...
export default {
  ...
  methods: {
    // 获取数据
    async getPlayerPayRemain() {
      const res = await playerPayRemain({
        page: this.currentPage,
        limit: this.pageSize
      })
      this.total = res.data.data.count
      this.payRemainData = res.data.data.data
    },
    ...
    async handleSearch() {
      try {
        ...
        const result = await playerPayRemain(query)
        this.payRemainData = result.data.data.data
      } catch (error) {
        console.error(error)
