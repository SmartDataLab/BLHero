    ...
    <el-card>
      ...
      <div>
        ...
        >
          ...
          <el-table-column prop="instanceId" label="实物的ID" min-width="200" />
          <el-table-column prop="instanceData" label="实物的数据" min-width="200" />
...
import { equipLogs } from '@/api/playerResourceLog'
    ...
    async handleInquire() {
      try {
        ...
        if (!this.serverText) {
          ...
          const result = await equipLogs(query)
          this.total = result.data.data.count
          this.equipData = result.data.data.data
        }
      } catch (error) {
        console.error(error)
