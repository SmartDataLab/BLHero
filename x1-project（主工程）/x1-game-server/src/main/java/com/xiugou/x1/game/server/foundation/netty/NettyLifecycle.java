...
public class NettyLifecycle implements Lifecycle {
	...
	public LifecycleInfo getInfo() {
		return LifecycleInfo.valueOf(this.getClass().getSimpleName(), Priority.LOW, Ordinal._25);
	}

	@Override
	public void start() throws Exception {
		ChannelInitializer<Channel> channelInitializer = null;
		channelInitializer = new WebSocketChannelInitializer(applicationSettings.getGameNettyIdle(), new NettyHandler());
		logger.info("WS未使用证书开启连接");
		
		//启动Netty
		netty = new NettySocketServer(2, 10);
		netty.startServer(applicationSettings.getGameServerPort(), channelInitializer);
		if(channelInitializer instanceof ByteDatagramChannelInitializer) {
			logger.info("服务器启动成功，TCP连接端口：{}", applicationSettings.getGameServerPort());
		} else if(channelInitializer instanceof WebSocketChannelInitializer) {
			logger.info("服务器启动成功，WS连接端口：{}", applicationSettings.getGameServerPort());
		}
	}
