 ...
 * 
 */
package org.gaming.db.mysql.table;
...
import org.gaming.db.annotation.enuma.IndexWay;
import org.gaming.db.mysql.dao.OriginDao;
import org.gaming.db.mysql.table.SqlDefine.IndexStruct;
import org.gaming.db.mysql.table.SqlDefine.MysqlVersion;
import org.gaming.db.mysql.table.SqlDefine.TableDes;
import org.gaming.db.mysql.table.SqlDefine.TableExist;
import org.gaming.db.mysql.table.SqlDefine.TableMaxId;
import org.gaming.db.mysql.table.SqlDefine.TableStatus;
import org.gaming.db.orm.AbstractEntity;
import org.gaming.db.orm.EntityMeta;
import org.gaming.db.orm.IndexMeta;
import org.gaming.db.orm.column.ColumnMeta;
import org.gaming.db.orm.column.PrimaryKeyColumnMeta;
...
public class TableBuilder {
	...
	public static void build(OriginDao<?> dao, String tableName)  {
		...
		//数据库的名字
		String dbName = dao.getDataBase().getDbConfig().getDbName();
		...
		if(!entityMeta.isAutoBuild() && !ddlSqls.isEmpty()) {
			for(String ddlSql : ddlSqls) {
				String errorMsg = String.format("在别名为%s的数据库中需要对名字为%s的表格作如下修正，请前往处理，参考语句如下：%s",
						dao.getDataBase().getAlias(), tableName, ddlSql);
				logger.warn(errorMsg);
			}
			String errorMsg = String.format("在别名为%s数据库中需要对名字为%s的表格进行修正，请参考上述日志",
					dao.getDataBase().getAlias(), tableName);
			logger.warn(errorMsg);
//			throw new RuntimeException(errorMsg);
	...
	private static List<String> updateAutoIncrement(OriginDao<?> baseDao, String tableName) {
		...
		if(mysqlVersion.getVersion().startsWith("8")) {
			String tableFullName = baseDao.getDataBase().getDbConfig().getDbName() + "/" + tableName;
