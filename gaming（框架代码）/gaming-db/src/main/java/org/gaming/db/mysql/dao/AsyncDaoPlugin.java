		...
		try {
			...
			for(T t : insertList) {
				try {
					...
					logger.error(String.format("retry insert %s data %s fail", asyncDao.getAsyncName(), GsonUtil.toJson(t)), e1);
				}
			}
		}
	}
		...
		try {
			...
			for(T t : updateList) {
				try {
					...
					logger.error(String.format("retry update %s data %s fail", asyncDao.getAsyncName(), GsonUtil.toJson(t)), e1);
				}
			}
		}
	}
